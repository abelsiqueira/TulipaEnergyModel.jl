...
"""
function validate_data!(connection, database_schema_name_override = Dict{Symbol,String}())
    ...
    for (log_msg, validation_function, fail_fast) in (
        ...
        @timeit to "$log_msg" append!(
            error_messages,
            validation_function(connection, database_schema_name_override),
        )
...
function _validate_has_all_tables_and_columns!(connection, database_schema_name_override = Dict())
    error_messages = String[]
    for (expected_schema_name, table_schemas) in TulipaEnergyModel.table_schemas
        schema_name = get(database_schema_name_override, expected_schema_name, expected_schema_name)
        for (table_name, table_schema) in table_schemas
            columns_from_connection = [
                row.column_name for row in DuckDB.query(
                    connection,
                    "SELECT column_name FROM duckdb_columns() WHERE table_name = '$table_name' AND schema_name = '$schema_name'",
                )
            ]
                ...
                has_table =
                    only([
                        row.count for row in DuckDB.query(
                            connection,
                            "SELECT COUNT(table_name) as count FROM duckdb_tables() WHERE table_name = '$table_name' AND schema_name = '$schema_name'",
                        )
                    ]) == 1
                if !has_table
                    push!(error_messages, "Table '$schema_name.$table_name' expected but not found")
                    continue
                end
            end
            ...
            for (column, _) in table_schema
                if !(column in columns_from_connection)
                    push!(
                        error_messages,
                        "Column '$column' is missing from table '$schema_name.$table_name'",
                    )
                end
            end
        end
...
function _validate_no_duplicate_rows!(connection, database_schema_name_override = Dict())
    ...
    input_schema = get(database_schema_name_override, "input", "input")
    cluster_schema = get(database_schema_name_override, "cluster", "cluster")
    duplicates = String[]
    for (schema_name, table_name, primary_keys) in (
        (input_schema, "asset", (:asset,)),
        (input_schema, "asset_both", (:asset, :milestone_year, :commission_year)),
        (input_schema, "asset_commission", (:asset, :commission_year)),
        (input_schema, "asset_milestone", (:asset, :milestone_year)),
        (input_schema, "assets_profiles", (:asset, :commission_year, :profile_type)),
        (input_schema, "assets_rep_periods_partitions", (:asset, :year, :rep_period)),
        (input_schema, "assets_timeframe_partitions", (:asset, :year)),
        (input_schema, "assets_timeframe_profiles", (:asset, :commission_year, :profile_type)),
        (input_schema, "flow", (:from_asset, :to_asset)),
        (input_schema, "flow_both", (:from_asset, :to_asset, :milestone_year, :commission_year)),
        (input_schema, "flow_commission", (:from_asset, :to_asset, :commission_year)),
        (input_schema, "flow_milestone", (:from_asset, :to_asset, :milestone_year)),
        (input_schema, "flows_profiles", (:from_asset, :to_asset, :year, :profile_type)),
        (
            input_schema,
            "flows_rep_periods_partitions",
            (:from_asset, :to_asset, :year, :rep_period),
        ),
        (input_schema, "group_asset", (:name, :milestone_year)),
        (cluster_schema, "profiles_rep_periods", (:profile_name, :year, :rep_period, :timestep)),
        (input_schema, "profiles_timeframe", (:profile_name, :year, :period)),
        (cluster_schema, "rep_periods_data", (:year, :rep_period)),
        (cluster_schema, "rep_periods_mapping", (:year, :period, :rep_period)),
        (cluster_schema, "timeframe_data", (:year, :period)),
        (input_schema, "year_data", (:year,)),
    )
        append!(
            duplicates,
            _validate_no_duplicate_rows!(connection, schema_name, table_name, primary_keys),
        )
    end
...
function _validate_no_duplicate_rows!(connection, schema_name, table_name, primary_keys)
    keys_as_string = join(primary_keys, ", ")
    duplicates = String[]
    for row in DuckDB.query(
        connection,
        "SELECT $keys_as_string, COUNT(*) FROM $schema_name.$table_name GROUP BY $keys_as_string HAVING COUNT(*) > 1",
    )
        values = join(["$k=$(row[k])" for k in primary_keys], ", ")
        push!(duplicates, "Table '$schema_name.$table_name' has duplicate entries for ($values)")
    end

    return duplicates
end

function _validate_schema_one_of_constraints!(connection, database_schema_name_override = Dict())
    error_messages = String[]
    for (expected_schema_name, table_schemas) in TulipaEnergyModel.table_schemas
        schema_name = get(database_schema_name_override, expected_schema_name, expected_schema_name)
        for (table_name, table) in table_schemas, (col, attr) in table
            if haskey(attr, "constraints") && haskey(attr["constraints"], "oneOf")
                ...
                for row in DuckDB.query(
                    connection,
                    "SELECT $col FROM $schema_name.$table_name WHERE $col NOT IN ($valid_types_string)",
                )
                    push!(
                        error_messages,
                        "Table '$schema_name.$table_name' has bad value for column '$col': '$(row[1])'",
                    )
                end
            end
        end
...
function _validate_only_transport_flows_are_investable!(
    connection,
    database_schema_name_override = Dict(),
)
    error_messages = String[]
    ...
    for row in DuckDB.query(
        ...
        FROM $input_schema.flow as flow
        LEFT JOIN $input_schema.flow_milestone as flow_milestone
...
function _validate_foreign_key!(
    ...
    query = "SELECT main.$column
        ...
        ANTI JOIN $foreign_schema_name.$foreign_table_name AS other
    ...
    for row in DuckDB.query(connection, query)
        push!(
            error_messages,
            "Table '$schema_name.$table_name' column '$column' has invalid value '$(row[1])'. Valid values should be among column '$foreign_key' of '$foreign_schema_name.$foreign_table_name'",
        )
    end
...
function _validate_group_consistency!(connection, database_schema_name_override = Dict())
    error_messages = String[]
...
function _validate_simple_method_data_consistency!(
    connection,
    database_schema_name_override = Dict(),
)
    error_messages = String[]
    _validate_simple_method_has_only_matching_years!(
        error_messages,
        connection,
        database_schema_name_override,
    )
    _validate_simple_method_all_milestone_years_are_covered!(
        error_messages,
        connection,
        database_schema_name_override,
    )
...
function _validate_simple_method_has_only_matching_years!(
    ...
    for row in DuckDB.query(
        ...
        FROM $input_schema.asset_both as asset_both
        LEFT JOIN $input_schema.asset as asset
        ...
        push!(
            error_messages,
            "Unexpected (asset='$(row.asset)', milestone_year=$(row.milestone_year), commission_year=$(row.commission_year)) in '$input_schema.asset_both' for an asset='$(row.asset)' with investment_method='$(row.investment_method)'. For this investment method, rows in '$input_schema.asset_both' should have milestone_year=commission_year.",
        )
    end
    ...
    for row in DuckDB.query(
        ...
        FROM $input_schema.flow as flow
        LEFT JOIN $input_schema.flow_both as flow_both
        ...
        push!(
            error_messages,
            "Unexpected (from_asset='$(row.from_asset)', to_asset='$(row.to_asset)', milestone_year=$(row.milestone_year), commission_year=$(row.commission_year)) in '$input_schema.flow_both' for an flow=('$(row.from_asset)', '$(row.to_asset)') with default investment_method='simple/none'. For this investment method, rows in '$input_schema.flow_both' should have milestone_year=commission_year.",
        )
    end
...
function _validate_simple_method_all_milestone_years_are_covered!(
    error_messages,
    connection,
    database_schema_name_override = Dict(),
)
    ...
    for row in DuckDB.query(
        ...
        FROM $input_schema.asset_milestone as asset_milestone
        LEFT JOIN $input_schema.asset as asset
            ON asset_milestone.asset = asset.asset
        LEFT JOIN $input_schema.asset_both as asset_both
        ...
        push!(
            error_messages,
            "Missing information in '$input_schema.asset_both': Asset '$(row.asset)' has investment_method='$(row.investment_method)' but there is no row (asset='$(row.asset)', milestone_year=$(row.milestone_year), commission_year=$(row.milestone_year)). For this investment method, rows in '$input_schema.asset_both' should have milestone_year=commission_year.",
        )
    end
    ...
    for row in DuckDB.query(
        ...
        FROM $input_schema.flow_milestone as flow_milestone
        LEFT JOIN $input_schema.flow as flow
        ...
        push!(
            error_messages,
            "Missing information in '$input_schema.flow_both': Flow ('$(row.from_asset)', '$(row.to_asset)') currently only has investment_method='simple/none' but there is no row (from_asset='$(row.from_asset)', to_asset='$(row.to_asset)', milestone_year=$(row.milestone_year), commission_year=$(row.milestone_year)). For this investment method, rows in '$input_schema.flow_both' should have milestone_year=commission_year.",
        )
    end
...
function _validate_use_binary_storage_method_has_investment_limit!(
    connection,
    database_schema_name_override = Dict(),
)
    error_messages = String[]
    ...
    for row in DuckDB.query(
        ...
        FROM $input_schema.asset_milestone as asset_milestone
        LEFT JOIN $input_schema.asset_commission as asset_commission
        ...
        push!(
            error_messages,
            "Incorrect investment_limit = $(row.investment_limit) for investable storage asset '$(row.asset)' with use_binary_storage_method = '$(row.use_binary_storage_method)' for year $(row.milestone_year). The investment_limit at year $(row.commission_year) should be greater than 0 in '$input_schema.asset_commission'.",
        )
    end
